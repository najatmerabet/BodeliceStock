...
@Component({
  ...
  template: `
    <div class="file-manager">
      ...
      <div class="fm-content" (dragover)="onDragOver($event)" (drop)="onDrop($event)" [class.drag-active]="isDragging">
        ...
        <!-- Folders Grid -->
        <div class="items-grid" *ngIf="folders.length > 0 || currentFolderId === null">
          <div class="folder-grid" *ngIf="folders.length > 0">
            <div class="folder-card" *ngFor="let folder of folders" (click)="openFolder(folder)" (contextmenu)="onFolderRightClick($event, folder)">
              ...
              <div class="folder-menu" (click)="$event.stopPropagation(); showFolderMenu(folder, $event)">
          ...
          <div class="files-grid" *ngIf="files.length > 0">
            <div class="file-card" *ngFor="let file of files" (contextmenu)="onFileRightClick($event, file)">
          ...
          <!-- Empty State -->
          <div class="empty-fm" *ngIf="folders.length === 0 && files.length === 0 && !loading">
      ...
      <div class="modal-overlay" *ngIf="showDeleteModal" (click)="showDeleteModal = false">
        <div class="modal-box modal-sm" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            <p *ngIf="deleteTarget">Voulez-vous vraiment supprimer <strong>{{ deleteTarget.nom }}</strong> ?</p>
          </div>
          <div class="modal-footer">
  ...
  styles: [`
    ...
    .folder-card:hover { background: #F8FAFC; }
    .folder-icon {
      width: 56px;
      height: 56px;
      border-radius: 12px;
    ...
    .folder-menu:hover { background: #E2E8F0; }
    .files-grid {
      display: flex;
      flex-direction: column;
      gap: 8px;
    ...
    .file-card:hover { border-color: #CBD5E1; background: #F8FAFC; }
    .file-icon {
      width: 44px;
      height: 44px;
      border-radius: 10px;
    ...
    .empty-fm .material-symbols-rounded { font-size: 64px; margin-bottom: 12px; }
    .empty-fm p { font-size: 1.1rem; font-weight: 600; color: #64748B; }
    .empty-fm .hint { font-size: 0.85rem; margin-top: 8px; }
    .fm-loading {
      display: flex;
      flex-direction: column;
      align-items: center;
    ...
    .modal-close { padding: 4px; border: none; background: none; cursor: pointer; border-radius: 6px; }
    .modal-close:hover { background: #F1F5F9; }
    .modal-body { padding: 20px; }
    .modal-body label { display: block; font-size: 0.85rem; font-weight: 500; margin-bottom: 6px; color: #475569; }
    .form-input {
      width: 100%;
      padding: 10px 12px;
      border: 1px solid #E2E8F0;
...
export class ClientFilesComponent implements OnInit {
  ...
  onFolderRightClick(event: Event, folder: ClientFolder): void {
    this.showFolderMenu(folder, event);
  }

  onFileRightClick(event: Event, file: ClientFileItem): void {
    event.stopPropagation();
  }
  ...
  getFileIconClass(type: string): string {
    ...
    if (ext.includes('jpg') || ext.includes('png') || ext.includes('image')) return 'image';
    if (ext.includes('doc') || ext.includes('word')) return 'word';
    if (ext.includes('xls') || ext.includes('excel') || ext.includes('sheet')) return 'excel';
    return '';
  }
